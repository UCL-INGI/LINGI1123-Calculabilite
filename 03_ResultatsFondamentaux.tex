...
\begin{mytheo}
  ...
  \begin{proof}
    Par construction de HALT : \\
    Soit nbStep(n,x)  = nombre d'instructions exécutées par $\phi_n(x)$.\\
    Cette fonction vaut $\bot$ si $P_n(x) = \bot$. Il s'agit donc d'une fonction partielle. Si cette fonction donnait toujours un résultat, cela reviendrait à calculer HALT.\\
    nbStepPrime(n,x) est l'extension totale de nbStep. Pour satisfaire le théorème précédent, il faudrait qu'elle renvoie toujours un résultat.\\
    Ce qui est impossible, sinon HALT devient calculable.\\
  \end{proof}
\end{mytheo}
