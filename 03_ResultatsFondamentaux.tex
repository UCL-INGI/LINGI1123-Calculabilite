...
<<<<<<< HEAD
  ...
  \begin{itemize}
    \item soit $\exists k \in      A $ tel que $\varphi_m = \varphi_k$
    \item soit $\exists k \in \bar{A}$ tel que $\varphi_n = \varphi_k$.
  \end{itemize}
  \begin{proof}
    ...
    En supposant A récursif, on arrive une contradiction.  En effet, 
=======
  il existe $i \in A$ et $j \in \bar{A}$ tel que $\phi_i = \phi_j$.
 
  \begin{proof}
    Soit un ensemble $A$ tel que $A \neq \emptyset$ et $\bar{A} \neq \emptyset$.  Il suffit de montrer que si 
    $\forall i \in A, \forall j \in \bar{A} : \phi_i \neq \phi_j$  (1) alors A est non récursif. \\
...
<<<<<<< HEAD
    ...
    La valeur $kE$ est-elle dans $A$ ou dans $\bar{A}$ ?
=======
    \[ \phi_{f(k)} = \phi_k \]
...
\begin{myrem}[Démonstration de $K$ grâce au point fixe]
	...
	\begin{itemize}
<<<<<<< HEAD
		\item $\varphi_1(x) = \bot \quad \forall \ x$
		\item $\varphi_0(x) = x \quad \forall  \ x$
		%\item $f(x) = $
		%	\begin{tabular}{l}
		%		i si $x\in K$\\
		%		j si $x\notin K$\\
...
>>>>>>> upstream/master
	...
	\end{itemize}
<<<<<<< HEAD
    On remarque que par construction pour tout $k$, $\varphi_{f(k)} \neq \varphi_k$.
    \begin{itemize}
      \item Si $k \in K$, $\varphi_k(k) \neq \bot$, $f(k) = 1$ et
        $\varphi_{f(k)}(k) = \varphi_1(k) = \bot \neq \varphi_k(k)$ donc $\varphi_{f(k)} \neq \varphi_k$.
      \item Si $k \notin K$, $\varphi_k(k) = \bot$, $f(k) = 0$ et
        $\varphi_{f(k)}(k) = \varphi_0(k) \neq \bot = \varphi_k(k)$ donc $\varphi_{f(k)} \neq \varphi_k$.
=======
    On montre que par construction pour tout $k$, $\phi_{f(k)} \neq \phi_k$.
    \begin{itemize}
      \item Si $k \in K$, $\phi_k(k) \neq \bot$, $f(k) = n$ et
        $\phi_{f(k)}(k) = \phi_n(k) = \bot \neq \phi_k(k)$ donc $\phi_{f(k)} \neq \phi_k$.
      \item Si $k \notin K$, $\phi_k(k) = \bot$, $f(k) = m$ et
