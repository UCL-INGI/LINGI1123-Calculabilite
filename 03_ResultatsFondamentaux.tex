...
\begin{enumerate}
	...
	\item \textbf{Contradiction} :
		\begin{enumerate}
			\item Comme \textit{diag'} est calculable dans Q, il existe un programme $Q_d$ qui calcule \textit{diag'}.  \\
			Mais quelle est la valeur de \textit{diag'(d)} ?
			\item Par définition de \textit{diag'}, on a $diag'(d) = interpret(d,d)+1$.
			\item Mais, comme $Q_d$ calcule $diag'$, par définition $diag'(d) = Q_d(d) = \phi_d(d) = interpret(d,d)$.
			En effet, calculer $\phi_d(d)$ revient à interpréter le programme
			d avec la donnée d.
			\item On obtient donc $interpret(d,d)+1 = interpret(d,d)$, ce qui est impossible car $interpret(d,d)$ est un entier. 
		\end{enumerate}
...
Le but est de montrer que \textbf{HALT} est récursif en construisant un programme qui décide \textbf{HALT}. Cela amène alors à une contradiction, car on sait que \textbf{HALT} n'est pas récursif. Ce qui implique donc que le programme n'est pas calculable.
...
while true do;
	...
	\item $\phi_i \neq \phi_j$ et $\forall i \in A, \forall j \in \stcomp{A}$ par hypothèse donc on peut remplacer $i$ et $j$ :
		$\phi_m \neq \phi_k$
...
P_m(z);
		...
		\begin{myrem}
			...
			$f(x)=\perp \ \forall x$ alors ce programme doit être dans $\stcomp{A}$ car $f(x) =\phi_k(x)$. Comme ce programme n'est pas récursif et que \textit{A} ne contient par hypothèse que des programmes récursifs, alors il doit forcément être dans $\stcomp{A}$. \\
	
			Or
			\begin{itemize}
				\item soit notre programme $P(z)$ ne se termine pas pour tout z, car $P_n(x)$ ne se termine pas, alors $g(z) = 		\phi_k(z)$. Donc $g(z)$ doit appartenir à $\stcomp{A}$ car $\phi_k(z)$ appartient à $\stcomp{A}$.
			\item soit notre programme $P(z)$ calcule $\phi_m(z)$ or on
				sait que $m\in A$, donc $g(z)$ appartient à \textit{A}.
			\end{itemize}
      En résumé : si $P_n(x)$ ne se termine pas, $P(z)$ sera dans
      $\stcomp{A}$ sinon $P(z)$ sera dans $A$.
		\end{myrem}
		
		On peut ainsi écrire un algorithme utilisant la description de $P(z)$ et dont la sortie (l'output) équivaut à calculer \textbf{HALT}.
...
respectent une propriété. Par exemple $A_1 = \{i| \phi_i \; \text{est totale}\}$ ou
