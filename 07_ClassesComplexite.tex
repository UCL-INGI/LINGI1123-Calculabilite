...
On va se ``limiter'' aux problèmes de décision.
...
algorithme d'un autre problème P. Déduire un algorithme de décision pour un ensemble E à partir d'un algorithme de décision d'un autre ensemble E'. Permet:
\begin{itemize}
...
Mais dans ce cours on ne va en étudier
que 3 :
\begin{itemize}
...
\begin{mydef}[$A$-complet]
	...
	\end{enumerate}
Autrement dit, un ensemble complet d'une classe d'ensembles est l'ensemble le "plus difficile" à décider parmi tous les ensembles de la classe. Si on trouve un algorithme pour cet ensemble, alors on peut reconnaître tous les autres ensembles de la classe.
\end{mydef}
...
\begin{myprop}
	Si $A$ est récursif alors pour tout $B$, $A \leq B$
\end{myprop}
...
Il y a deux manières de travailler en théorie de la complexité. Premièrement, la théorie classique qui utilise les machines de Turing pour calculer la complexité car la notion de transitions est extrêmement claire.
On peut vraiment calculer facilement le nombre de transitions. Ensuite Java, pour lequel cela dépend du compilateur et d'autres éléments, ce qui implique que ce n'est pas toujours très précis. Les théoriciens préfèrent donc les machines de Turing car la définition de nombre d'instructions est très précise. Mais cette manière est peu intuitive et on va donc se permettre d'utiliser le concept standard de notation grand O en Java pour calculer la complexité. Chaque modèle a ses inconvénients et ses avantages et nous utiliserons le plus facile en fonction du problème.\\

Il est important de se rappeler une thèse vu dans le chapitre précédant qui dit que tous les modèles de complexité ont entre eux des complexités spatiales et temporelles reliées de façon polynomiale. Ceci implique que tout problème pratiquement faisable dans un modèle (non déterministe) le sera également dans tous les autres modèles (non déterministe) !
...
\begin{myrem}
La complexité n'a pas de sens pour les ensembles récursivement énumérables, car ceux-ci ne se terminent pas toujours. Or, la complexité n'a de sens que pour les programmes qui se terminent !
\end{myrem}
\paragraph{Classes basées sur le modèle non déterministe}
\begin{mydef}[NTIME(f)]
	Famille des ensembles ND-récursifs pouvant être décidés par un programme
...
\begin{myrem}
	La classe P ne dépend pas du modèle de calcul car on sait que entre deux modèles de calcul, les différences sont uniquement polynomiales. Résultat très positif car classe de problèmes qui est la même peu importe le modèle utilisé !
\end{myrem}
...
\begin{myrem}
	La classe NP ne dépend pas du modèle de calcul (même raison que pour la classe P).
\end{myrem}
...
\begin{myprop}
	...
	Le $c$ est le facteur de branchement.
\end{myprop}
...
\begin{myrem}
	On a montré, à l'aide de la propriété $7.36$, que la complexité temporelle est plus importante que la complexité spatiale car on réussit à avoir une borne sur la spatiale à l'aide de la temporelle ! Cela justifie en pratique pourquoi on se concentre souvent d'abord sur la complexité temporelle.
\end{myrem}

\begin{myrem}
	On a également démontré avec la propriété $7.38$ que la complexité spatiale n'apporte aucune information sur la complexité temporelle car une complexité en $O(c^f)$ est beaucoup beaucoup trop grande.
\end{myrem}
...
\subseteq NP$ (car un programme déterministe est un cas particulier d'un programme non déterministe)  mais on n'a pas encore montré si oui ou non $NP\subseteq P$. Si c'était le cas, ce serait génial car un tas de problèmes dans plein d'applications de la vie de tous les jours appartiennent à la classe NP. Et cela signifirait qu'on serait capable de les résoudre en temps polynomial !
...
\paragraph{} On va montrer que $SAT$ (= Satisfaction des formules propositionnelles) est $NP$-complet en 2 parties :
\begin{enumerate}
	\item $SAT \in NP$
...
\begin{myexem} Soit les trois variables $A$, $B$ et $C$
	...
	$(\neg A \lor B)$, $(B \lor C)$ et $(A \lor \neg C)$ sont les trois clauses de notre formule. Elles ont chacune deux litéraux.
	La formule est ici satisfaite si $A=false$, $B=true$ et $C=false$. D'autres solutions sont possibles.
\end{myexem}

\subsection{$SAT \in NP$}
Il existe un programme ND polynomial capable de décider si $x\in SAT$. On pose
...
\begin{itemize}
	\item Problème du circuit hamiltonien HC (trouver un circuit qui passe
		une seule fois par tous les sommets)
	\item Problème du voyageur de commerce TS (trouver un circuit qui relie
...
\section{$P = NP$ ?}
On peut faire une similitude avec la thèse de Church-Turing. Beaucoup d'efforts ont été réalisés pour trouver des algorithmes polynomiaux pour des problèmes NP-complets mais sans jamais de succès. Tous furent des échecs.
Le débat fait toujours rage aujourd'hui dans le monde scientifique et il y a de fortes présomptions que de tels algorithmes n'existent pas. Donc forte présomption que $P \neq NP$. Aucune preuve n'existe à ce jour et la question fait partie des questions les plus importantes à résoudre dans le monde scientifique avec une somme de 1 million d'euro à la clé.
