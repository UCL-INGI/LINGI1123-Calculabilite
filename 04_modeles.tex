...
\begin{myprop} \label{prop:ND-eq}
	...
	\begin{proof}
		...
		Cependant les branches pouvant être infinies, il est nécessaire d'explorer chaque branche avec la même profondeur.
		On dit qu'on explore l'arbre en largeur (\textit{Breadth First Search}). La complexité du programme ainsi construit devient alors exponentielle.
	\end{proof}
\end{myprop}
\paragraph{Exemple} : \\
...
\begin{myprop}
	...
	\begin{proof}
		...
		Selon la définition \ref{def:recursif}, l'ensemble A est récursif.
...
\begin{myprop}
	...
	\begin{proof}
		...
		énumérable.
		En effet, s'il est ND-récursivement
		énumérable, il existe un ND-programme dont le comportement est décrit à la définition \ref{def:ND-recenum}.
		...
		Selon la définition \ref{def:recursivement enum}, l'ensemble A est récursivement énumérable.

		Inversement, si un ensemble A quelconque est récursivement énumérable, alors il est ND-récursivement énumérable. En effet, par définition, il existe un programme D se comporte comme décrit dans la définition \ref{def:recursivement enum}.
...
\begin{figure}
	\centering
	\includegraphics[width=0.4\textwidth]{Images/567px-ChevreLoupSalade.jpg}
...
\begin{myprop}
	Le modèle des fonctions primitives récursives n'est un modèle complet de la calculabilité.
\end{myprop}
...
\begin{myexem}\ \\
	...
	\end{align}
	On multiplie n par m et on rajoute n. La complexité sera ici (n*m), cela revient à faire des +1 tout le temps.
	\begin{align}
	mult (m, 0) &= 0 \\
