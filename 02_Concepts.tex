...
Notation :
\begin{itemize}
    \item Une \emph{chaîne de caractère} ou un \emph{mot} : séquence \textbf{\underline{finie}} de symboles. Ex:
		\subitem -- $abceced$, 
		\subitem -- $010101101$
...
\begin{itemize}
	...
	\item S'il n'existe pas de $b \in B$ tel que $f(a)=b$ alors $f(a)$ est indéfini,
		$f(a) = \perp$\footnote{La notation $\perp$ se dit \emph{bottom}.}
\end{itemize}
...
\begin{itemize}
	\item Par \emph{un texte fini} déterminant sans contradiction ni ambigüité le contenu
		de la table.
...
\begin{myprop}
 Tout ensemble infini de chaines fini de caractères est énumérable.
 \begin{proof}
  Si dans un ensemble infini de caractères ont prend un ensemble fini de chaines de caractères, cet ensemble est énumérable. 
 \end{proof}
\end\{myprop}
...
\begin{myprop}
 ...
 \begin{proof}
  Java est constitué d'un ensemble fini de chaine de caractères et est donc énumérable.
 \end{proof}
\end\{myprop}
...
\begin{myprop}
 L'ensemble des fonctions calculable est énumérable.
 \begin{proof}
  Une fonction calculable est une fonction pour laquelle il existe un programme pour le résoudre et est donc énumérable.
 \end{proof}
\end\{myprop}
...
\begin{myprop}
 ...
 \begin{proof}
 Ont sais que un ensemble est énumérable ou dénombrable s'il est fini ou s'il a le même cardinal que $\mathbb{\N}$. \\
 Un language est un ensemble de chaine de caractères fini donc est énumerable.
 \end{proof}
\end\{myprop}
